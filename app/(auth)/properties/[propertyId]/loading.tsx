import Container from "@/components/Container";

export default function Loading() {
    return (
        <Container>
            <div className="flex justify-center">
                {/* Sidebar Skeleton */}
                <div className="hidden sm:flex flex-col sticky top-21.25 pr-4 pl-0 py-10 min-w-62.5 h-fit">
                    <nav>
                        <ul className="flex flex-col gap-2">
                            {[...Array(5)].map((_, i) => (
                                <li key={i} className="px-4 py-3 flex items-center gap-3 rounded-full">
                                    <div className="h-6 w-6 bg-neutral-100 rounded animate-pulse" />
                                    <div className="h-5 w-28 bg-neutral-100 rounded animate-pulse" />
                                </li>
                            ))}
                        </ul>
                    </nav>
                    <div className="mt-5">
                        <div className="h-11 w-full bg-neutral-100 rounded-full animate-pulse" />
                    </div>
                    <div className="mt-5">
                        <div className="h-10 w-full bg-neutral-100 rounded-full animate-pulse" />
                    </div>
                </div>

                {/* Main Content Skeleton */}
                <div className="flex flex-col sm:p-8 sm:pt-12 w-full gap-5 sm:border-l-2 border-gray-200 animate-pulse">

                    <div className="flex flex-col gap-5 sm:gap-8">
                        {/* Heading */}
                        <div className="h-9 w-48 bg-neutral-100 rounded-lg" />

                        {/* Name */}
                        <div className="flex sm:items-center gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-16 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="h-10 w-full bg-neutral-100 rounded-xl" />
                        </div>

                        {/* Custom URL */}
                        <div className="flex sm:items-center gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-24 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="h-10 w-full bg-neutral-100 rounded-xl" />
                        </div>

                        {/* Description */}
                        <div className="flex sm:items-start gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-24 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="h-40 w-full bg-neutral-100 rounded-xl" />
                        </div>

                        {/* Terms & Conditions by Host */}
                        <div className="flex sm:items-start gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-48 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="h-40 w-full bg-neutral-100 rounded-xl" />
                        </div>

                        {/* Category */}
                        <div className="flex sm:items-center gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-20 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="h-10 w-full bg-neutral-100 rounded-xl" />
                        </div>

                        {/* Listed Services */}
                        <div className="flex sm:items-start gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="sm:w-1/3 space-y-1">
                                <div className="h-5 w-28 bg-neutral-100 rounded" />
                                <div className="h-4 w-48 bg-neutral-100 rounded" />
                            </div>
                            <div className="flex flex-wrap gap-2 w-full">
                                {[...Array(6)].map((_, i) => (
                                    <div key={i} className="h-9 w-24 bg-neutral-100 rounded-full" />
                                ))}
                            </div>
                        </div>

                        {/* Price */}
                        <div className="flex sm:items-center gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-14 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="h-10 w-full bg-neutral-100 rounded-xl" />
                        </div>

                        {/* Location */}
                        <div className="flex sm:items-center gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-20 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="h-10 w-full bg-neutral-100 rounded-xl" />
                        </div>

                        {/* Images */}
                        <div className="flex gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-32 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="w-full grid grid-cols-4 gap-3">
                                {[...Array(4)].map((_, i) => (
                                    <div key={i} className="aspect-square bg-neutral-100 rounded-xl" />
                                ))}
                            </div>
                        </div>

                        {/* Amenities */}
                        <div className="flex gap-1 sm:gap-10 flex-col sm:flex-row">
                            <div className="h-5 w-24 bg-neutral-100 rounded sm:w-1/3" />
                            <div className="grid grid-cols-3 gap-3 w-full">
                                {[...Array(6)].map((_, i) => (
                                    <div key={i} className="h-8 bg-neutral-100 rounded-lg" />
                                ))}
                            </div>
                        </div>

                        {/* Save button */}
                        <div className="flex justify-end pt-5">
                            <div className="h-10 w-20 bg-neutral-100 rounded-lg" />
                        </div>
                    </div>
                </div>
            </div>
        </Container>
    );
}
