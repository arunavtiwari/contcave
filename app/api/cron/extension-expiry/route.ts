...
import { expireExtensionRequests } from "@/lib/maintenance/postBooking";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const results = await expireExtensionRequests(200, undefined, automationStart);
    return createSuccessResponse({ total: results.length, results });
  } catch (error) {
    return handleRouteError(error, "GET /api/cron/extension-expiry");
  }
}
