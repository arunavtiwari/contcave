...
import { autoCompleteCheckedInReservations } from "@/lib/maintenance/postBooking";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const results = await autoCompleteCheckedInReservations(200, undefined, automationStart);
    assertNoFailedMaintenanceResults(results);
    return createSuccessResponse({ total: results.length, results });
  } catch (error) {
    return handleRouteError(error, "GET /api/cron/auto-complete-reservations");
  }
}
