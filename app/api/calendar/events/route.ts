...
async function refreshCalendarAccessToken(account: {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Failed to parse token refresh response: ${responseText.substring(0, 200)}`
      );
    }
    ...
    if (!response.ok) {
      const errorMessage =
        typeof refreshedTokens === "object" &&
          ...
          ? String((refreshedTokens as { error?: unknown }).error)
          : "Unknown error";
    ...
    if (
      ...
      typeof (refreshedTokens as { access_token?: unknown }).access_token !==
      "string"
    ) {
      throw new Error("Invalid token refresh response: missing access_token");
    }

    return refreshedTokens as {
      access_token: string;
      expires_in?: number;
      refresh_token?: string;
    };
    ...
    try {
      ...
      if (isInvalidCredentials && googleAccount && googleAccount.refresh_token) {
        try {
          ...
          throw new Error(
            `Failed to refresh calendar access token: ${errorMessage}. Original error: ${error instanceof Error ? error.message : "Unknown"
            }`
          );
        }
      } else {
